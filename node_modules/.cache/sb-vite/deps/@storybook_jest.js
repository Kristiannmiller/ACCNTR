...
function deprecate(name2, replacementText) {
  console.warn(
    `Warning: ${name2} has been deprecated and will be removed in future updates.`,
    replacementText
  );
}
...
function toHaveAccessibleErrorMessage(htmlElement, expectedAccessibleErrorMessage) {
  ...
  const error = normalize(
    ((_a2 = htmlElement.ownerDocument.getElementById(errormessageId)) == null ? void 0 : _a2.textContent) ?? ""
  );
  return {
...
function buildElementRoleList(elementRolesMap) {
  function makeElementSelector({ name: name2, attributes }) {
    return `${name2}${attributes.map(({ name: attributeName, value, constraints = [] }) => {
...
function printAttribute(stringify, name2, value) {
  return value === void 0 ? name2 : `${name2}=${stringify(value)}`;
}
function getAttributeComment(stringify, name2, value) {
  return value === void 0 ? `element.hasAttribute(${stringify(name2)})` : `element.getAttribute(${stringify(name2)}) === ${stringify(value)}`;
}
function toHaveAttribute(htmlElement, name2, expectedValue) {
  checkHtmlElement(htmlElement, toHaveAttribute, this);
  const isExpectedValuePresent = expectedValue !== void 0;
  const hasAttribute = htmlElement.hasAttribute(name2);
  const receivedValue = htmlElement.getAttribute(name2);
  return {
    pass: isExpectedValuePresent ? hasAttribute && this.equals(receivedValue, expectedValue) : hasAttribute,
    message: () => {
      const to = this.isNot ? "not to" : "to";
      const receivedAttribute = hasAttribute ? printAttribute(this.utils.stringify, name2, receivedValue) : null;
      const matcher = this.utils.matcherHint(
        `${this.isNot ? ".not" : ""}.toHaveAttribute`,
        "element",
        this.utils.printExpected(name2),
        {
      ...
      return getMessage(
        ...
        printAttribute(this.utils.stringify, name2, expectedValue),
        "Received",
        receivedAttribute
      );
    }
...
function isSubset(styles, computedStyle) {
  return !!Object.keys(styles).length && Object.entries(styles).every(([prop, value]) => {
    ...
    return spellingVariants.some(
      (name2) => computedStyle[name2] === value || computedStyle.getPropertyValue(name2) === value
    );
  });
}
...
function getFormValue(container, name2) {
  const elements = [...container.querySelectorAll(`[name="${(0, import_css.default)(name2)}"]`)];
...
function getAllFormValues(container) {
  ...
  return names.reduce(
    (obj, name2) => ({
      ...obj,
      [getPureName(name2)]: getFormValue(container, name2)
    }),
    {}
  );
}
function toHaveFormValues(formElement, expectedValues) {
  ...
  return {
    pass: Object.entries(expectedValues).every(
      ([name2, expectedValue]) => compareAsSet(formValues[name2], expectedValue)
    ),
    message: () => {
      const to = this.isNot ? "not to" : "to";
  ...
  constructor() {
    ...
    let start = ({ storyId, playUntil }) => {
      ...
      if (!call && shadowCall && Object.values(resolvers).length > 0) {
        let nextId = (_a2 = this.getLog(storyId).find((c2) => c2.status === "waiting")) == null ? void 0 : _a2.callId;
  ...
  invoke(fn, call, options) {
    ...
    let handleException = (e3) => {
      if (e3 instanceof Error) {
        let { name: name2, message, stack, callId = call.id } = e3, exception = { name: name2, message, stack, callId };
...
var name = "default";
var _a;
expect.extend((_a = matchers_exports == null ? void 0 : matchers_exports[name]) != null ? _a : matchers_exports);
export {
  expect,
  jest
};
