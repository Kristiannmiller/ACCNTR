...
function createEvent2(type3, target, init) {
  ...
  const { EventType, defaultInit } = eventMap2[type3];
...
// node_modules/@storybook/testing-library/node_modules/@testing-library/user-event/dist/esm/document/patchFocus.js
...
function isAcceptableFile(file, accept) {
  ...
  return normalize2(accept).trim().split(/\s*,\s*/).some((acceptToken) => {
    if (acceptToken.startsWith(".")) {
      return normalize2(file.name).endsWith(acceptToken);
    } else if (wildcards.includes(acceptToken)) {
      return normalize2(file.type).startsWith(acceptToken.replace("*", ""));
    }
    return normalize2(file.type) === acceptToken;
  });
}
